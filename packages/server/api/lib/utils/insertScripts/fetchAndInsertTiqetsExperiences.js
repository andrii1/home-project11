...
const insertExperiences = require('./insertExperiencesTiqets'); // your DB insert function
...
async function fetchPage(page) {
  const url = `https://api.tiqets.com/v2/experiences?page=${page}&page_size=${pageSize}`;
  try {
...
async function fetchAndInsertAllProducts() {
  ...
  while (page < 3) {
    ...
    // Insert into DB
    await insertExperiences(products);
